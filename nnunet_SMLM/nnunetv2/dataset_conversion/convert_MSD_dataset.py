...
def split_4d_nifti(filename, output_folder):
    ...
    if dim == 3:
        shutil.copy(filename, join(output_folder, file_base[:-7] + "_0000.tif"))
        return
    elif dim != 4:
        ...
        for i, t in enumerate(range(img_npy.shape[0])):
            ...
            sitk.WriteImage(img_itk_new, join(output_folder, file_base[:-7] + "_%04.0d.tif" % i))
...
def convert_msd_dataset(source_folder: str, overwrite_target_id: Optional[int] = None,
    ...
    with multiprocessing.get_context("spawn").Pool(num_processes) as p:
        ...
        source_images = [i for i in subfiles(imagesTr, suffix='.tif', join=False) if
        ...
        source_images = [i for i in subfiles(imagesTs, suffix='.tif', join=False) if
                         not i.startswith('.') and not i.startswith('_')]
        ...
        # copy segmentations
        source_images = [i for i in subfiles(labelsTr, suffix='.tif', join=False) if
                         not i.startswith('.') and not i.startswith('_')]
...
if __name__ == '__main__':
    convert_msd_dataset(r'E:\pyCharm\Project\nnUNet-master\nnUNet_raw\target01_SMLM', overwrite_target_id=2)
